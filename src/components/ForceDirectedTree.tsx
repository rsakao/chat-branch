...
export default function ForceDirectedTree({
  ...
  // 会話ペアを作成する関数
  const createNodes = useCallback((messages: Record<string, Message>): ConversationNode[] => {
      ...
      if (userMsg.parentId) {
        ...
        if (parentMsg?.role === 'assistant') {
          ...
          if (grandParentUserMsg) {
            const parentNode = nodes.find(n => n.userMessage.id === grandParentUserMsg.id)
  ...
  // リンクを作成する関数
  const createLinks = useCallback((nodes: ConversationNode[], messages: Record<string, Message>): TreeLink[] => {
  ...
  useEffect(() => {
    ...
    // Force simulation
    const simulation = d3.forceSimulation<ConversationNode>(nodes)
      .force('link', d3.forceLink<ConversationNode, TreeLink>(links)
        .id(d => d.id)
        .distance(100)
        .strength(0.8))
    ...
    const node = g.append('g')
      ...
      .attr('r', (d) => {
        ...
        const hasChildren = d.aiMessage?.children && d.aiMessage.children.length > 0
        return hasChildren ? baseSize + 5 : baseSize
      })
      .attr('fill', (d) => {
        const isActive = currentMessageIds.has(d.userMessage.id) || 
    ...
    // ドラッグ機能
    const drag = d3.drag<SVGCircleElement, ConversationNode>()
  ...
  }, [messages, currentMessageIds, dimensions, isDarkMode, createNodes, createLinks, onSelectMessage])
